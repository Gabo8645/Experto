...
function buildSummary() {
  ...
  html += `<p>Horario: ${selectedSchedule}${selectedSchedule==='programado' && scheduleDateInput.value ? ` el ${scheduleDateInput.value}` : ''}</p>`;
  html += `<p><strong>Total: $${selectedService==='auto' ? totalCarCostEl.textContent : totalCost.textContent}</strong></p>`;
  html += `<button id="btnProceedToPayment" class="btn">Ir a pago</button>`;
...
// Botón de resumen
btnCalculateTotal.addEventListener('click', buildSummary);
document.getElementById('btnNextFromCars')?.addEventListener('click', buildSummary);
